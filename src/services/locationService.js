...
export const startLocationBroadcast = (driverId, onError, options = {}) => {
  ...
  // FIX 1: Initialize driver_locations immediately to avoid race condition
  const initializeLocation = async () => {
    ...
    if (overrideLocation?.lat && overrideLocation?.lng) {
      try {
        ...
        console.error('Initial location setup error:', err)
        onError?.(err.message)
      }
      return
    }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        console.error('Geolocation error during initialization:', error)
...
// FIX 4: Helper functions to track driver busy status
export const setDriverBusyStatus = async (driverId, isBusy) => {
  /**
   * Set driver busy status when accepting/completing rides
   * This prevents the driver from being matched for new rides
   */
  try {
    ...
    console.log(`[Location] Driver ${driverId} busy status: ${isBusy}`)
  } catch (err) {
    console.warn(`Could not update busy status for driver ${driverId}:`, err)
  }
}
